...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/12/24 18:37:11 by kahantar          #+#    #+#             */
/*   Updated: 2018/12/24 18:40:39 by kahantar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		handle_fat(void *ptr, t_info info, char *name)
{
	...
	while (i < nb_arch)
	{
		if (swap_bit(arch->cputype) == CPU_TYPE_X86_64)
			ft_otool(ptr + swap_bit(arch->offset), info, name);
		if ((arch = incrementing((void*)arch, &info,\
				sizeof(arch), sizeof(arch))) == NULL)
			return (-1);
		i++;
	}
	return (0);
